TGanalysis <- function(chcemat, scoreList, noption, sumscr_rng=NULL, 
  ...
  for (icycle in 1:ncycle) {
    HALsave[icycle,1] <- parmListvec[[icycle]]$parmList$meanF
    HALsave[icycle,2] <- parmListvec[[icycle]]$infoList$infoSurp
  }
  ...
  #  set up objects affected by switching to arc length as the
  #  plotting abscissa

  infofine      <- infoList$infofine
