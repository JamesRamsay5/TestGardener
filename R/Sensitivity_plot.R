Sensitivity_plot <- function(scrfine, SfdList, Qvec, dataList, plotindex=1:n, 
  ...
  for (i in plotindex)
  {
    ...
    p <- plotDS(Mi, scrfine, DSfitfinei, Qveci, keyi, plotrange, alltype, 
...
plotDS   <- function(Mi, scrfine, DSfitfinei, Qvec, keyi=NULL, plotrange=c(0,100),
                     alltype=TRUE, mssplot=FALSE, DSrng, ttllab, optVec=NULL,
                     ttlsz=NULL, axisttl=NULL, axistxt=NULL, lgdlab=NULL)
{
  value    <- 0
  variable <- 0
  ...
  if (!is.null(keyi))
  {
    ...
    dffine_r <- data.frame(scrfine=scrfine,
                           DSfitfinei_r=DSfitfinei[,keyi])
    
    dwp <- ggplot2::ggplot(data=dffine_r, ggplot2::aes(scrfine,DSfitfinei[,keyi])) +
    ...
    dffine <- data.frame(scrfine=scrfine, DSfitfinei_r = DSfitfinei[,ind])
