chce_simulate <- function(index.pop, SfdList) {
#  CHCE_SIMULATE generates an Nindex.pop by N matrix of random score index
...
#  Last modified on 16 November 2023 by Jim Ramsay
  ...
  chce_mat <- matrix(0,nindex.pop,n)
  for (i in 1:n) {
    SListi <- SfdList[[i]]
    Mi     <- SListi$M
